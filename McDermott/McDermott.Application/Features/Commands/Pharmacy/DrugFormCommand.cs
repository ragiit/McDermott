...
namespace McDermott.Application.Features.Commands.Pharmacy
{
    public class DrugFormCommand
    {
        ...
        public class GetDrugFormQuery(Expression<Func<DrugForm, bool>>? predicate = null, int pageIndex = 0, int? pageSize = 10, string? searchTerm = "", bool removeCache = false) : IRequest<(List<DrugFormDto>, int pageIndex, int pageSize, int pageCount)>
        {
            public Expression<Func<DrugForm, bool>> Predicate { get; } = predicate!;
            public bool RemoveCache { get; } = removeCache!;
