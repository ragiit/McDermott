...
{
    public partial class DoctorScheduledSlotPage
    {
        private List<DoctorScheduleSlotDto> DoctorScheduleSlots { get; set; } = [];
        ...
        private async Task OnDelete(GridDataItemDeletingEventArgs e)
        {
            try
            {
                ...
                {
                    await Mediator.Send(new DeleteDoctorScheduleSlotRequest(SelectedDataItems[0].Adapt<DoctorScheduleSlotDto>().Id));
                }
                else
                {
                    var a = SelectedDataItems.Adapt<List<DoctorScheduleSlotDto>>();
                    await Mediator.Send(new DeleteDoctorScheduleSlotRequest
                    {
        ...
        private async Task LoadData(int pageIndex = 0, int pageSize = 10)
        {
            try
            {
                PanelVisible = true;
                SelectedDataItems = [];
                var a = await Mediator.Send(new GetDoctorScheduleSlotQuery
                {
                    Includes =
                    [
                       x => x.Physician,
        ...
        private bool IsPopUpDoctorScheduleSlot = false;

        private async Task GenerateScheduleDoctor_Click()
        {
            IsPopUpDoctorScheduleSlot = true;

            if (DoctorScheduleSlot.PhysicianId is 0 && Physicians.Count > 0)
                DoctorScheduleSlot.PhysicianId = Physicians.FirstOrDefault()?.Id ?? 0;
        }
        ...
        private async Task EditItem_Click()
        {
            try
            {
                ...
                var a = (Grid.GetDataItem(FocusedRowVisibleIndex) as DoctorScheduleSlotDto ?? new());

                PanelVisible = true;
                var results = await Mediator.Send(new GetUserQueryNew
                {
                    Predicate = x => x.Id == a.PhysicianId,
                    Select = x => new User
                    {
                        Id = x.Id,
                        Name = x.Name,
        ...
        private async Task SaveScheduleEditSlot(GridEditModelSavingEventArgs e)
        {
            try
            {
                PanelVisible = true;
                var editModel = (DoctorScheduleSlotDto)e.EditModel;

                editModel.DayOfWeek = editModel.StartDate.DayOfWeek.ToString();

                if (editModel.Id == 0)
                    //await Mediator.Send(new CreateDoctorScheduleSlotRequest(editModel));
                    ToastService.ShowInfo("Please select the schedule");
                else
                    await Mediator.Send(new UpdateDoctorScheduleSlotRequest(editModel));

                SelectedDataItems = [];
                await LoadData(activePageIndex, pageSize);

                ToastService.ShowInfo("Successfully update the schedule");
            }
            catch (Exception ex)
            {
                ex.HandleException(ToastService);
        ...
        private async Task SaveScheduleSlot()
        {
            try
            {
                ...
                ValidationResult results = new CreateUpdateDoctorScheduleSlotValidator().Validate(DoctorScheduleSlot);
