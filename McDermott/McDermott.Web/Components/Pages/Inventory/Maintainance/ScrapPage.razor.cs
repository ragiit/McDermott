...
{
    public partial class ScrapPage
    {
        ...
        private async Task LoadData(int pageIndex = 0, int pageSize = 10)
        {
            try
            {
                ...
                PostProduct = GetProduct.FirstOrDefault() ?? new();
                ...
                if (PageMode == EnumPageMode.Update.GetDisplayName())
                {
                    ...
                    var resultMaintainance = await Mediator.Send(new GetMaintainanceQuery(x => x.EquipmentId == PostProduct.Id, pageSize: pageSize, pageIndex: pageIndex));
                    GetMaintainanceScrap = resultMaintainance.Item1.Where(x => x.Status == EnumStatusMaintainance.Scrap).ToList();
