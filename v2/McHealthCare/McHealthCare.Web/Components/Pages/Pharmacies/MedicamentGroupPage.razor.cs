...
namespace McHealthCare.Web.Components.Pages.Pharmacies
{
    public partial class MedicamentGroupPage
    {
        ...
        private List<DoctorDto> Phy = new();
            ...
            try
            {
                if (product is not null)
                {
                    ...
                    else
                    {
                        ...
                        FormMedicamenDetails!.FrequencyId = ChekMedicament?.FrequencyId ?? Guid.Empty;
                    }
        ...
        private async Task OnDelete(GridDataItemDeletingEventArgs e)
        {
            try
            {
                ...
                List<Guid> id = a.Select(x => x.Id).ToList();
                List<Guid> detailIdsToDelete = new();
                ...
                if (MGForm.Id == Guid.Empty)
                {
                    try
                    {
                        ...
                        if (medicamentGroupDetails.Any(x => x.MedicamentId == FormMedicamenGroupDetailss.MedicamentId))
                            return;
                        if (FormMedicamenGroupDetailss.Id == Guid.Empty)
                        {
