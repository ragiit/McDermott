    ...
    public partial class EmailTemplatePage
    {
        ...
        private async Task LoadUser()
        {
            try
            {
                ...
                var Partner = await UserService.GetAllPatients();
                EmailCc = [.. Partner!.Select(x => x.ApplicationUser!.Email)];
                ToPartner = [.. Partner.ToList()];
        ...
        private async Task OnSave(GridEditModelSavingEventArgs e)
        {
            try
            {
                var EmailFormTemplate = e.EditModel.Adapt<EmailTemplateDto>();
                ...
                if (EmailFormTemplate.Id == Guid.Empty)
                {
