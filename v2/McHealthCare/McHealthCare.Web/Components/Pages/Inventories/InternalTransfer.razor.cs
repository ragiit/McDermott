namespace McHealthCare.Web.Components.Pages.Inventories
{
    public partial class InternalTransfer
    {
        ...
        private IReadOnlyList<object> SelectedDataItemsDetail { get; set; } = [];
        ...
        {
            if (FormInternalTransfer.SourceId == Guid.Empty || FormInternalTransfer.SourceId == null)
            {
        ...
        private async Task UpdateProductDetailsAsync(List<TransferStockProductDto> items, Guid? sourceLocationId)
        {
            ...
            try
            {
                ...
                if (FormInternalTransfer is not null)
                {
                    ...
                    foreach (var items in checkTranferProduct)
                    {
                        ...
                        FormTransactionStocks.Quantity = -(items.QtyStock * Convert.ToInt64(Cek_Uom?.BiggerRatio));
        ...
        private async Task OnSave_Detail(GridEditModelSavingEventArgs e)
        {
            ...
            if (FormInternalTransfer.Id == Guid.Empty)
            {
                /*
