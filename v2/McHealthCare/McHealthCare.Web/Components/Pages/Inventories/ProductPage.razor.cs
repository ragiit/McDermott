    ...
    public partial class ProductPage
    {
        ...
        private async Task NewItem_Click()
        {
            ...
            FormProductDetails = new ProductDetailDto
            {
                ...
                SalesPrice = 100,
                Tax = "11%",
                UomId = Uoms.FirstOrDefault(x => x.Name == "Unit")?.Id ?? Guid.Empty
            };
        ...
        private async Task EditItem_Click(ProductDto? p = null)
        {
            try
            {
                ...
                if (FormProductDetails.UomId == Guid.Empty)
                {
                    FormProductDetails.UomId = Uoms.FirstOrDefault(x => x.Name == "Unit")?.Id ?? Guid.Empty;
        ...
        private async Task OnDelete(GridDataItemDeletingEventArgs e)
        {
            try
            {
                ...
                {
                    List<Guid> MProductId = SelectedDataItems.Adapt<List<ProductDto>>().Select(x => x.Id).ToList();
        ...
        private async Task OnSave()
        {
            try
            {
                ...
                if (FormProductDetails.Id == Guid.Empty)
                {
        ...
        {
            FormMedicaments.Id = FormProductDetails.MedicamentId ?? Guid.Empty;
            FormMedicaments.ProductId = getProduct?.Id ?? Guid.Empty;
        ...
        private async Task CreateNewProductAndMedicament()
        {
            ...
            if (FormProductDetails.HospitalType == "Medicament")
            {
                ...
                if (FormMedicaments.Id == Guid.Empty)
                {
        ...
        private async Task onSaveStock()
        {
            try
            {
                ...
                if (FormStockProduct.Id == Guid.Empty)
                {
